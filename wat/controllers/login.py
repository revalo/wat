...
def login_page():
    ...
    try:
        if (
            ...
            and request.args["state"] == session["state"]
        ):
            r = requests.post(
            ...
            if (
                "email" in user_info
                and user_info["email_verified"] == True
                and user_info["email"].endswith("@mit.edu")
            ):
                # Authenticated
                ...
                if user is None:
                    ...
                    user = User(
                        email=email, name=name, last_activity=datetime.datetime.now()
                    )
                    db.session.add(user)
                    db.session.commit()
        ...
        return render_template(
            "error.html",
            login_url=login_url,
            error="Stuff didn't go according to plan :(",
        )
